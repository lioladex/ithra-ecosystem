...
const CORE_MEMORIES = [
    ...
    },
    {
        ...
        prompt: "Photorealistic 3D render, cinematic film-VFX quality, dynamic high-angle wide shot in 16:9 widescreen cinematic composition, looking almost straight down over a sunlit forest clearing at midday, bright hard overhead light casting short, sharp, high-noon shadows. The single most important visual element in this image is a huge, dark, sharply-defined shadow shape stretched across the bright sunlit dirt of the clearing, cast by something large and winged flying just out of frame overhead — a long body-shadow with two wide wing-shadows angled back from it, unmistakably shaped like a bird or bat in flight, racing across the ground and about to sweep directly over the herd below; render this ground-shadow itself as a bold, high-contrast, unmissable dark silhouette on the pale dirt, the clearest single shape in the whole composition, even though the creature casting it is entirely out of frame above. A herd of exactly seven tall horse-sized alien grazers — small beaked heads, emerald-green glowing eyes, tall translucent leaf-shaped ears — has just seen this shadow: every head is snapped sharply upward and back toward it in the same instant, and every one of their ribbon-like solar ruffs is already collapsed and folded flat against the neck rather than fanned open, since the herd is only ever seen with these unfurled while calmly grazing, never while alarmed or moving. The whole herd is caught mid-turn, wheeling together as one away from the shadow's path, dust exploding under their hooves. In the distance at the treeline, exactly four lean dog-sized wolf-like predators with thick necks and glowing orange-red cheek stripes are shown from behind, already mid-retreat with their backs turned to the herd and their bodies angled away into the treeline shadows, tails low, clearly withdrawing rather than approaching or chasing. No other creatures of any kind are present in the scene beyond these eleven animals and the passing shadow. Dust kicked up by the herd's sudden turn catches the overhead sunbeams, subsurface scattering on the translucent ear-membranes, motion blur on the wheeling herd, shallow depth of field with the retreating predators softly blurred in the background, full scene visible, no text, no illustration, no painting, no concept art style."
    },
    {
        chapter: 4,
