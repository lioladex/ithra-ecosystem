...
const CORE_MEMORIES = [
    ...
    },
    {
        ...
        prompt: "Photorealistic 3D render, cinematic film-VFX quality, true top-down bird's-eye shot in 16:9 widescreen cinematic composition, camera looking straight down at the forest clearing floor from directly above, bright hard midday overhead light casting short, sharp shadows directly beneath each animal. In the foreground, closer to the camera and occupying the lower half of the frame, exactly four lean dog-sized wolf-like predators with thick necks and glowing orange-red cheek stripes stand still on a patch of freshly disturbed, churned-up dirt where their ambush just failed, their body language reading as frustrated and let down: ears flattened back, heads and tails hanging low, shoulders slumped, one pawing uselessly at the empty churned dirt where the prey should have been, another snapping its jaws shut on nothing, none of them moving to give chase any further. Cutting diagonally across the sunlit dirt between the foreground predators and the background, a huge, dark, sharply-defined shadow shape is cast on the ground by something large and winged flying just out of frame above the camera — a long body-shadow with two wide wing-shadows angled back from it, unmistakably shaped like a bird or bat in flight; render this ground-shadow as a bold, high-contrast, unmissable dark silhouette against the pale dirt, clearly readable from this straight-down angle as the reason the hunt just failed. In the background, occupying the upper part of the frame and smaller with distance, a herd of exactly seven tall horse-sized alien grazers — small beaked heads, emerald-green glowing eyes, tall translucent leaf-shaped ears, their ribbon-like solar ruffs collapsed and folded flat against their necks rather than fanned open — is already well clear of the ambush site, running together in a tight wheeling group toward the treeline, dust trailing behind them. No other creatures of any kind are present in the scene beyond these eleven animals and the passing shadow. Subsurface scattering on the translucent ear-membranes of the fleeing herd, motion blur on their running legs in the distance, shallow depth of field keeping the frustrated predators in the foreground sharp while the fleeing herd softens with distance, full scene visible, no text, no illustration, no painting, no concept art style."
    },
    {
        chapter: 4,
